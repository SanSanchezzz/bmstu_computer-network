...
void Socket::setNonBlocked(bool opt) const throw(std::exception) {
    set_non_blocked_impl(m_Sd, opt);
}

void Socket::setRcvTimeout(int sec, int microsec) const throw(std::exception) {
    struct timeval tv;
    tv.tv_sec = sec;
    tv.tv_usec = microsec;
...
void Socket::send(const std::string& str) const throw(std::exception) {
...
std::string Socket::recv_loop() const throw(std::exception) {
    char buf[256];
    std::string ret;
    while (true) {
#ifdef __APPLE__
...
bool Socket::hasData() const throw(std::exception) {
    char buf[1];
...
std::shared_ptr<Socket> Socket::accept() const throw(std::exception) {
